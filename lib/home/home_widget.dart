  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      ...
      if (FFAppState().selectedLanguageCode == 'en') {
        ...
        _model.newOutPutFrench = await UmaruGroup.getCategoryInFrenchCall.call(
          token: FFAppState().token,
        );
        FFAppState().update(() {
          FFAppState().categoryData = UmaruGroup.getCategoryInFrenchCall
              .frenchProductCategories(
                (_model.newOutPutFrench?.jsonBody ?? ''),
              )!
              .toList();
          FFAppState().isLoading = false;
        });
                        ...
                        if (FFAppState().selectedLanguageCode == 'fr')
                          Padding(
                            padding:
                                EdgeInsetsDirectional.fromSTEB(0, 24, 0, 10),
                            child: FutureBuilder<ApiCallResponse>(
                              future: UmaruGroup.bestSellerInFrenchCall.call(),
                              builder: (context, snapshot) {
                                // Customize what your widget looks like when it's loading.
                                if (!snapshot.hasData) {
                                  return Center(
                                ...
                                final frenchlist2HomepageBestSellerInFrenchResponse =
                                    snapshot.data!;
                                return Builder(
                                  builder: (context) {
                                    final bestsellerChild = getJsonField(
                                      frenchlist2HomepageBestSellerInFrenchResponse
                                          .jsonBody,
                                      r'''$''',
                                    ).toList().take(4).toList();
                                    return GridView.builder(
